...
export class Migration1762321438041 implements MigrationInterface {
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      `CREATE INDEX "idx_user_middle_name" ON "users" ("middle_name") `,
    );
    await queryRunner.query(
    ...
    await queryRunner.query(
      `CREATE UNIQUE INDEX "idx_user_rfc" ON "users" ("rfc") WHERE "rfc" IS NOT NULL`,
    );
    await queryRunner.query(
      `CREATE UNIQUE INDEX "idx_user_curp" ON "users" ("curp") WHERE "curp" IS NOT NULL`,
    );
    await queryRunner.query(
      `CREATE INDEX "idx_user_status" ON "users" ("status") `,
    );
    await queryRunner.query(
      `CREATE TABLE "sms_validations" ("id" uuid NOT NULL DEFAULT uuid_generate_v4(), "user_id" uuid NOT NULL, "sms_provider" character varying(50), "phone" character varying(50), "status" character varying(50), "sms_service_sid" character varying(100), "sms_request_sid" character varying(100), "sms_action" character varying(50), "sms_status" character varying(50), "sms_request_date" TIMESTAMP WITH TIME ZONE, "created_at" TIMESTAMP WITH TIME ZONE NOT NULL DEFAULT now(), "updated_at" TIMESTAMP WITH TIME ZONE NOT NULL DEFAULT now(), CONSTRAINT "PK_36c3a5fd2e8728d056313c2e2d9" PRIMARY KEY ("id"))`,
    );
    await queryRunner.query(
      `CREATE INDEX "idx_sms_validation_user_id" ON "sms_validations" ("user_id") `,
    );
    await queryRunner.query(
      `CREATE INDEX "idx_sms_validation_phone" ON "sms_validations" ("phone") `,
    );
    await queryRunner.query(
      `CREATE INDEX "IDX_e99332d7f43d8cfac190644441" ON "sms_validations" ("status") `,
    );
    await queryRunner.query(
      `CREATE INDEX "idx_sms_validation_sms_service_sid" ON "sms_validations" ("sms_service_sid") `,
    );
    await queryRunner.query(
      `CREATE INDEX "idx_sms_validation_sms_request_sid" ON "sms_validations" ("sms_request_sid") `,
    );
    await queryRunner.query(
    ...
    await queryRunner.query(
      `ALTER TABLE "sms_validations" ADD CONSTRAINT "FK_cb2c66c3835c99f76a16e5abb02" FOREIGN KEY ("user_id") REFERENCES "users"("id") ON DELETE NO ACTION ON UPDATE NO ACTION`,
    );
  }

  public async down(queryRunner: QueryRunner): Promise<void> {
    await queryRunner.query(
      `ALTER TABLE "sms_validations" DROP CONSTRAINT "FK_cb2c66c3835c99f76a16e5abb02"`,
    );
    await queryRunner.query(
      `ALTER TABLE "addresses" DROP CONSTRAINT "FK_95c93a584de49f0b0e13f753630"`,
    );
    await queryRunner.query(
      `DROP INDEX "public"."idx_sms_validation_sms_request_sid"`,
    );
    await queryRunner.query(
      `DROP INDEX "public"."idx_sms_validation_sms_service_sid"`,
    );
    await queryRunner.query(
      `DROP INDEX "public"."IDX_e99332d7f43d8cfac190644441"`,
    );
